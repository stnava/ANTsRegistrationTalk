...
1.  Start with a *rigid transformation*:  $I(x) \approx J(R(x))$ s.t. negative $MI(I,J,R)$ is minimized

2.  Follow by an *affine transformation*:  $I(x) \approx J(A(R(x)))$ s.t. negative $MI(I,J,A \circ R)$ is minimized with fixed $R$

3.  Finally, a *diffeomorphism*:  $I(x) \approx J(\phi(A(R(x))))$ s.t. $k$-neighborhood correlation $CC_k(I,J,A \circ R)$ is minimized with fixed $R, A$
